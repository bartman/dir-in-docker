...
RUN apt-get update && \
    apt-get install -y curl ca-certificates sudo neovim jq git kitty-terminfo \
        locales ripgrep fd-find systemd-coredump linux-perf \
        ${EXTRA_PACKAGES} && \
    apt-get clean && \
...
# omp global config / state live under ~/.omp/agent/
RUN mkdir -p /home/$USERNAME/.local/bin /home/$USERNAME/.omp/agent
...
# install omp prebuilt binary (standalone fork, no bun/npm/pi)
